...
from app.models import STRATEGY_STATUSES, TASK_STATUSES, WORKSTREAM_STATUSES, StrategyItem, Task, Workstream
...
def _status_badge(status: str) -> None:
    with ui.row().classes("items-center gap-1"):
        ui.element("div").classes("w-2.5 h-2.5 rounded-full").style(
            f"background-color: {STATUS_COLORS.get(status, STATUS_COLOR_FALLBACK)}"
        )
        ui.label(status).classes("text-sm")


def _load_strategy_items() -> list[dict]:
...
def _load_workstreams() -> list[dict]:
    """All workstreams (unfiltered) with lead resolved; strategy_item_id kept
    for grouping into each strategy item's section (None -> "Unassigned")."""
...
def _save_strategy(row_id, name, description, status, priority, owner_id, start, end) -> None:
    with get_session() as session:
...
def _save_workstream(row_id, name, description, strategy_id, status, lead_id, start, end) -> None:
    with get_session() as session:
...
def build() -> None:
    ...
    def refresh() -> None:
        ...
        with content_holder:
            strategy_items = _load_strategy_items()
            all_workstreams = _load_workstreams()

            by_strategy: dict[int, list[dict]] = {}
            unassigned: list[dict] = []
            for w in all_workstreams:
                bucket = unassigned if w["strategy_item_id"] is None else by_strategy.setdefault(
                    w["strategy_item_id"], []
                )
                bucket.append(w)

            for item in strategy_items:
                _render_strategy_card(item, by_strategy.get(item["id"], []))
    ...
    def _render_workstream_table(workstreams: list[dict]) -> None:
        ...
        if not visible:
            msg = "No workstreams yet." if not workstreams else "No workstreams match the current filter."
            ui.label(msg).classes("text-sm text-gray-500 pl-2")
            return
        table = ui.table(columns=WORKSTREAM_COLUMNS, rows=visible, row_key="id").classes("w-full")
        ...
        table.on("edit", lambda e: open_workstream_form(e.args))
        table.on("timeline", lambda e: show_timeline(e.args))

    def _render_strategy_card(item: dict, workstreams: list[dict]) -> None:
        with ui.card().classes("w-full gap-3"):
            with ui.row().classes("items-start justify-between w-full"):
                with ui.column().classes("gap-1"):
                    ...
                    ui.label(
                        f"Owner: {item['owner'] or '—'}  ·  Priority: {item['priority'] or '—'}  ·  "
                        f"{item['target_start'] or '—'} → {item['target_end'] or '—'}"
                    ).classes("text-xs text-gray-500")
                ui.button(icon="edit", on_click=lambda i=item: open_strategy_form(i)).props("flat dense")

            _render_workstream_table(workstreams)

            ui.button(
                "Add workstream", icon="add",
                on_click=lambda sid=item["id"]: open_workstream_form({"strategy_item_id": sid}),
            ).props("flat dense")

    def _render_unassigned_card(workstreams: list[dict]) -> None:
        with ui.card().classes("w-full gap-3 bg-gray-50"):
            ui.label("Unassigned").classes("text-lg font-bold text-gray-600")
            ui.label("Workstreams not linked to a strategy item.").classes("text-xs text-gray-500")
            _render_workstream_table(workstreams)
            ui.button(
                "Add workstream", icon="add",
                on_click=lambda: open_workstream_form({"strategy_item_id": None}),
            ).props("flat dense")
    ...
    def open_strategy_form(row: dict | None = None) -> None:
        row = row or {}
        with ui.dialog() as dialog, ui.card().classes("w-96"):
            ...
            def save() -> None:
                if not name.value.strip():
                    ...
                    return
                _save_strategy(row.get("id"), name.value.strip(), description.value, status.value,
    ...
    def open_workstream_form(row: dict | None = None) -> None:
        row = row or {}
        with ui.dialog() as dialog, ui.card().classes("w-96"):
            ui.label("Edit workstream" if row.get("id") else "Add workstream").classes("text-lg font-bold")
            name = ui.input("Name", value=row.get("name", "")).classes("w-full")
            description = ui.textarea("Description", value=row.get("description") or "").classes("w-full")
            ...
            def save() -> None:
                if not name.value.strip():
                    ui.notify("Name is required", type="negative")
                    return
                _save_workstream(row.get("id"), name.value.strip(), description.value, strategy.value,
